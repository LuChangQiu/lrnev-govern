# lrnev 执行顺序总纲（2026-06-12 定稿，2026-07-06 勾进度；3.0.0 已完成见第 0 条，2026-09-07）
...
## 0. 3.0.0（2026-09-07）已完成——scene04 标准化战役 ✅

> lrnev 3.0.0 = governance scene `04-ai-guidance-standardization` 端到端标准化战役收口：六 spec（01-00 语义权威模型 → 06-00 文档体系）、T-001~T-029 全任务闭环、B0~B4 五轮证据对照（B0 摘录基线 → B3 sha-c 真机对照 → B4 sha-d 发布内容快照复测）、T-006 字段裁决（Profile 结构化挂载负决策回退）、`--profile core|full` 工具面分层（L7）与 docs 收敛。主交付：MCP 响应双通道契约（structuredContent canonical 信封 + 逐工具渲染文本，text 通道破坏性变更）、五角色引导语义体系、`decision_context` 客户端声明通道、业务拒绝统一 isError、错误路径统一转义。
> B3/B4 真机收官：G5 归档边界生效（E-06a 归档率 4/5→0/5）、opencode E-02 5/5（0 -32602 三连）、E-06b 真实续接首测 4/5、claude E-02 四 SHA 0/5 终确认（激励缺口 → 裁决 1.4 不阻塞发布）。
> **详细见 CHANGELOG 3.0.0**——本文为 v2.1~v2.3 期（2026-06-12~07-06）执行总纲，3.0.0 战役进度以 CHANGELOG 为准，不再逐条展开。
