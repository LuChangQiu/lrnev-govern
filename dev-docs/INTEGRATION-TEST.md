...
>
> 当前实况：契约面已由全量 `npm test`（**1070 条 / 80 文件**，2026-09-10 基准，见"十三、发布门禁与自动化覆盖"）锁死；3.0.0 发布前的真机对照（T-027 B3/B4）已在 claude-code / opencode 上完成一轮，判定结论与录制件位置见"十、T-027 真机对照资产"。本清单是**发布前真机走查基线**：至少在一个真实客户端上把"三、黄金路径"与"九、体验层"过一遍，其余条目按变更面抽查——已被 T-027 判定的项不必重复盲测。
>
...
npm test                 # 全量 vitest：1070 条 / 80 文件（2026-09-10 基准，以实跑为准）
...
> ⚠️ 上面的 1070 条全绿**不等于 F-14 通过**。协议层（真实客户端握手/呈现）、ai_followup 是否真驱动 AI、真机体验，是 CI **测不到**的部分，必须在真实客户端手动走一遍。
>
