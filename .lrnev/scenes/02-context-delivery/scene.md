...
**包含**：
- 阶段一（spec 01-00，已完成，2026-06-18 completed）：维护态小增量落位 + 需求审核门 + 任务启动锚点回填 + 分流边界（F-01~F-04）。
- 阶段二（spec 02-00，已完成，2026-06-18 completed，硬依赖 01-00）：定位升级——治理地图 + context_search 锚点级抽段 + BM25 打分。
- 阶段三（spec 03-00，已完成，2026-06-18 completed）：`lrnev report`——读自有 `.lrnev` 数据生成 validates 覆盖率 / 需求→任务→完成链路 / release notes 草稿，第一个「分红」。
