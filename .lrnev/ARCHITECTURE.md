...
- `src/core/`：确定性业务 Manager——Scene / Spec / Task / Gate / ADR / Errorbook / Memory / Summarizer / Searcher / AgentRegistry / ClaimStore / Doctor / HookManager / ProjectStatus / GovernanceMap / GovernanceReport / GoalAssessor / WorkspaceManager / SessionCommit 等；引导语义模块：SpecGuidance（spec_get 引导）、GateGuidance（gate followup）、guidance-semantics（语义角色类型）、decision-context（客户端声明核对）
