...
- 开发/测试：`vitest`（1079 测试，unit/integration/e2e）、`tsx`、`tsc`；tests 另有独立类型门禁 `typecheck:test`（tsconfig.test.json）
...
- **行为变更须有测试**：既有 `.lrnev/` 数据持续可读，变更须覆盖——当前 1079 测试 + `typecheck:test` 门禁。
