...
- **引导措辞收敛（真机驱动）**：开发/扩展功能请求 → 先 `task_create` 登记再实施（直接编辑 requirements/design 只限需求细化/文档维护，不能替代登记——E-02 四 SHA 0/5 直写观测驱动）；用户决定优先条款显式化；状态机提示与合法回退（failed/blocked→pending）表述修正；**spec 收口动作点引导**——task_update 本次更新使该 spec 全部任务 completed 且 spec 尚未收口（status ≠ completed）时，followup 追加收口动作点（先 `spec_gate_check(gate=completion)` 验证收口条件、通过后 `spec_update` 把 spec 状态回填 completed），堵"任务全绿但 spec 停在 draft"的漏收口（T-027 判定驱动，47fb686，2026-09-07）。
...
- **文档（3.0.0 动作量最大的一次）**：README 全量重写（中文单语、双通道契约与破坏性迁移说明、常驻提示词单源化指向 AI-ADAPTATION、全绝对链接）；AI-ADAPTATION 更新（`--profile` 用法、工具档位、常驻模板 A/B）与实测矩阵溯源修正（执行者/客户端版本号去无依据信息、"42 个 MCP 工具全调通"改注口径差异：报告实际注入 38、42 为注册全集，表尾补各报告来源注记）；ARCHITECTURE 目录树（补 `src/schemas/`、`docs/examples/` 两行）与 .lrnev PROJECT/ARCHITECTURE 治理文档刷新到 3.0.0；发布前终审 P0-P2 文档族修正（CONFIG 删除错误的"连接期缓存"表述，改口径为"MCP 进程长驻时改配置后重启 MCP 生效"；HOOKS 注明 `lrnev_hook_*` 五工具仅 `--profile full` 注册、core 档裁剪且配置走 CLI；GOVERNANCE-FLOW/MULTI-AGENT 语义修正——`--scan` 定位为占位 flag（行为同默认 init，候选 Scene 生成属规划中能力）、状态机补 draft→archived 合法路径、修正对未单独成文的"Agent 存活信号改进程生命周期"ADR 的两处引用改指 CHANGELOG [1.2.0]/[2.3.0] 条目；CONTRIBUTING 补发布门禁清单——Node ≥ 20、npm ci（package-lock.json 已入库）、typecheck/typecheck:test/npm test/build 四道门禁）；dev-docs 研究档案收敛（11 份被取代报告归档、2 个异常快照删除、ai-discussions 嵌套镜像移除回本机区）与 2026-09 文件治理收尾——ai-guidance-standardization 战役档案按域物理分组（deliverables/evidence/notes，路径即引用键，src 注记与 scripts/tests 证据引用同步）、报告/决策文件名中文化；审定决策档案镜像层 `dev-docs/decisions/`（08-28~09-07 审定定稿镜像累计 73 份 = 首批 09-04~09-07 裁决/汇总 17 份 + 补批 08-28~09-03 早期定稿 56 份；2026-09 收尾按内容分层：裁决/汇总/规格 37 份留存本目录，36 份纯逐任务"复审通过"过程背书归档 `dev-docs/archive/decisions/`；文件名去 `DeepSeek-` 作者前缀——发布声称的 GitHub 可核验依据）；06-00 spec 交付物（client-integration-guide/mcp-response-conformance）为研究期草案且与 3.0.0 实现不符，保持 dev-docs 研究档案定位（正式客户端文档待 06-00 收口时重写，2026-09-07 裁决 ADR 0002）。
...
- `HookEvent` 类型联合补 `task.update.pending`——docs/HOOKS 事件表与 TaskManager 运行时（blocked/failed 回退 pending 路径）均支持该事件，类型声明漏项（纯加宽、无运行时行为变化，hook 27 测试全绿）。
