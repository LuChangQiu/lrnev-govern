...
export const TOOL_DESCRIPTIONS = {
  ...
  task_create: '追加单条结构化 Task。何时用：临时补一个执行项；一次拆多条任务清单请改用 task_create_many；大项可用 parent 拆子任务；在已有 in-progress/ready Spec 下登记新增执行项/开发增量时也用它（不是只有新建 Spec 后才拆任务）。前置：spec 已创建。例子：用户在已有 Spec 说"继续补充登录功能" → task_create{scene:"01-user-management",spec:"01-00-user-login",title:"补充登录功能"}。',
  task_create_many: '一次原子性创建多个 Task。何时用：spec ready 后一次性拆任务清单；补单个仍用 task_create。批内依赖用 key 临时键(禁 T-xxx)，任一条失败整批不写、返回全部错误。例子：task_create_many{tasks:[{key:"a",title:"建模"},{title:"接口",depends_on:["a"]}]}。',
  ...
  agent_register: '注册当前 Agent，返回 agent_id；未传时自动生成。何时用：stdio 启动时自动注册;脚本化或跨主机会话手动调用。返回 data.gc 仅在本次顺手清理了死 agent/过期 claim 时出现，没有该字段=无需清理，正常。',
