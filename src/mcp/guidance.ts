...
export const WORKFLOW_OVERVIEW = [
  ...
  '治理契约（已 lrnev_init）：工作前先读 .lrnev/steering/；ai_followup 必须执行、不确定先查档案、不擅自生成 ADR/Memory。',
...
export const TOOL_DESCRIPTIONS = {
  ...
  project_status: '[核心] 返回接手快照。何时用：继续已有项目或新会话开头。前置：已 init。例子：project_status{} → 查看 active tasks、ADR 和 open errors。',
  governance_map: '[核心] 返回治理地图：scene→spec(状态/L0)→锚点标题 的压缩全景（只读、只含标题级，不含正文）。何时用：接手项目或需要一次性了解全貌、按 URI 直接跳转时，替代反复 context_search + 读全文。',
  ...
  scene_list: '[核心] 列出当前工作区中的所有 Scene。何时用：接手项目、选择工作场景或排查 broken Scene 时。',
  scene_get: '读取一个 Scene 的元信息和统计信息。何时用：需要确认某个 Scene 的文档与统计概况时。',
  spec_create: '[核心] 创建 Spec 三文档。何时用：先自问"这是可独立交付、能写出 WHEN…THEN 验收的特性吗"——通常适合为这类特性开 spec；做完没有独立验收可挂的小改动(改文档/排版/注释、小重构、调参数、答问题等，举例非穷举)直接做、不要开 spec。拿不准先问用户、别默认开。注意：以上是建议，若用户已明确要求（如"帮我新建一个 Spec"），即使已有相似 Spec 可以承载，也应尊重用户决定直接创建。前置：已 init；scene 可省略。例子：spec_create{name:"login"}。',
  spec_list: '[核心] 列出指定 Scene 下的所有 Spec。何时用：查看同 Scene 已有哪些特性、避免重复或找接手目标。',
  spec_get: '[核心] 读取一个 Spec 的元信息和三文档存在性。何时用：进入某个 Spec 前确认 requirements/design/tasks 是否齐全。',
  spec_update: '[核心] 按状态机更新 Spec 状态(draft→ready→in-progress→completed→archived)。何时用：gate 通过后回填状态，或开重写版后把被取代的旧版标 archived(归档后其待办不再进可领列表)。【决策边界】archived 是终态且影响可审计性：不要替用户做归档决定——用户改主意（如改用旧 Spec）不构成自动归档依据，仅在用户明确要求归档时执行。',
  spec_gate_check: '[核心] 运行 Spec gate。何时用：创建后、需求填完或任务完成后验收。前置：spec 已存在；ready 前替换 FILL。例子：spec_gate_check{gate:"ready"} → 返回 checks。',
  task_create: '[核心] 追加单条结构化 Task。何时用：临时补一个执行项；一次拆多条任务清单请改用 task_create_many；大项可用 parent 拆子任务；在已有 in-progress/ready Spec 下登记新增执行项/开发增量时也用它（不是只有新建 Spec 后才拆任务）。前置：spec 已创建。例子：用户在已有 Spec 说"继续补充登录功能" → task_create{scene:"01-user-management",spec:"01-00-user-login",title:"补充登录功能"}。',
  task_create_many: '[核心] 一次原子性创建多个 Task。何时用：spec ready 后一次性拆任务清单；补单个仍用 task_create。批内依赖用 key 临时键(禁 T-xxx)，任一条失败整批不写、返回全部错误。例子：task_create_many{tasks:[{key:"a",title:"建模"},{title:"接口",depends_on:["a"]}]}。',
  task_update: '[核心] 按状态机更新 Task。何时用：开始/完成/阻塞任务时。前置：task 已存在，遵守 pending→in_progress→completed。例子：task_update{task_id:"T-001",status:"completed"}。',
  ...
  context_search: '[核心] 目录优先检索 context:// 资源，优先使用 L0/L1 摘要。何时用：找相关 Spec、ADR、记忆或错误记录时。',
  ...
  agent_register: '[自动] 注册当前 Agent，返回 agent_id；未传时自动生成。何时用：stdio 启动时自动注册;脚本化或跨主机会话手动调用。返回 data.gc 仅在本次顺手清理了死 agent/过期 claim 时出现，没有该字段=无需清理，正常。',
  agent_heartbeat: '[自动] 更新 Agent last_heartbeat 并续租其 claim。何时用:通常无需(存活随进程自动判定);仅跨主机协作需要兜底续活时调用。',
  agent_list: '[自动] 列出 Agent 注册表，并惰性计算 active/dead 状态。何时用：查看当前有哪些客户端会话在线时。',
  agent_unregister: '[自动] 注销 Agent 会话。何时用：Agent 正常退出或交接完成后清理会话状态。',
  lrnev_doctor: '检查 .lrnev 工作区结构、Spec、Task、ADR、claim、hook 和 context；可迁移旧 TODO、清理遗留摘要。何时用：gate 失败、broken 条目、数据异常或接手前。',
  lrnev_hook_list: '[配置] 列出 hook 配置和最近执行状态。何时用：确认 hooks.json 生效情况或排查自动化未触发时。',
  lrnev_hook_trigger: '[配置] 手动触发 hook 事件，用于测试 hooks.json 配置。何时用：新增或修改 hook 后做本地验证时。',
  lrnev_hook_tail_log: '[配置] 读取最近 hook 执行日志。何时用：hook 失败、超时、warning 或 trigger 后确认输出时。',
  lrnev_hook_enable: '[配置] 启用 hooks.json 中指定 hook。何时用：临时关闭的自动化需要恢复时。',
  lrnev_hook_disable: '[配置] 禁用 hooks.json 中指定 hook。何时用：某个 hook 失败、太慢或干扰主流程时。',
} as const;
...
const GUIDE_SECTIONS: Record<GuideTopic, GuideSection> = {
  workflow: {
    ...
    content: [
      ...
      'ready gate 通过后：按 design 拆任务——已想好完整任务清单用 task_create_many 一次原子创建（批内依赖用 key），临时补单个用 task_create；执行前用 task_update 标 in_progress，完成后标 completed。',
      '拆 Task 时自查：是否大到可分别认领/并行？子步骤是否要各自独立验收？否则保持单个 Task，别为拆而拆；需要拆时用 task_create(parent=父任务)。',
      DESIGN_DECISION_EXAMPLE,
      '收尾：所有任务完成后跑 spec_gate_check(completion)，通过后用 spec_update 把状态回填 completed，再回看 L0 摘要与验收标准。',
      '接手已有项目：先调用 project_status，看 active_tasks、recent ADR、open errors，再从 in_progress task 继续；也可调 governance_map 看 scene→spec→锚点全景、lrnev_report 看治理欠债体检。',
    ].join('\n'),
  },
  tools: {
    title: '工具速查',
  ...
  concepts: {
    ...
    content: [
      ...
      DESIGN_DECISION_EXAMPLE,
      'Spec id 里的 VV 是正式重写版号，不是修订号；用户请求的开发/扩展功能（加/实现/补充某能力）先 task_create 登记再实施，直接编辑只用于需求细化/文档维护（不能替代开发任务登记）；只有整体推翻重做且需要保留旧版对照时，才用 spec_create 的 version=1/2/... 开新版。',
