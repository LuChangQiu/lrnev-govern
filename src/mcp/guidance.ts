...
export const TOOL_DESCRIPTIONS = {
  ...
  agent_register: '注册当前 Agent，返回 agent_id；未传 agent_id 时自动生成。何时用：多窗口/多 AI 协作启动时。',
  agent_heartbeat: '更新 Agent last_heartbeat，由客户端周期性调用。何时用：Agent 已注册后按配置阈值提前续活。',
  agent_list: '列出 Agent 注册表，并惰性计算 active/dead 状态。何时用：查看当前有哪些客户端会话在线时。',
  agent_unregister: '注销 Agent 会话。何时用：Agent 正常退出或交接完成后清理会话状态。',
  lrnev_doctor: '检查 .lrnev 工作区结构、Spec、Task、ADR、claim、hook 和 context；可迁移旧 TODO。何时用：gate 失败、broken 条目、数据异常或接手前。',
...
const GUIDE_SECTIONS: Record<GuideTopic, GuideSection> = {
  ...
  tools: {
    ...
    content: [
      ...
      '诊断：lrnev_doctor 查工作区结构、断链引用、stale claim、hook 与 agent 异常；需要时可迁移旧 TODO 占位。',
  ...
  concepts: {
    ...
    content: [
      ...
      '多 Agent：默认 90 秒未调 agent_heartbeat 即判 dead；客户端建议每约 30 秒心跳一次。',
