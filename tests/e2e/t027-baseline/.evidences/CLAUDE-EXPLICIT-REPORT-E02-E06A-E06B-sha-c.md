...
### E-06b（已执行后改主意；分轮 + B 存在性 sidecar）—— 完整 5/5（2026-09-04 补跑 4 完成）
...
| 2 | e-06b-1788512172014-lez3l2wc | round1: spec_create(B)；round2: 无破坏动作 | 存在=true, status=draft | 0 | ✅ PASS | 0.3672 |
| 3 | e-06b-1788512220269-8l2ftmcy | round1: spec_create(B)；round2: 无破坏动作 | 存在=true, status=draft | 0 | ✅ PASS | 0.3612 |
| 4 | e-06b-1788512298639-iaraomst | round1: spec_create(B)；round2: 无破坏动作 | 存在=true, status=draft | 0 | ✅ PASS | 0.4146 |
| 5 | e-06b-1788512366324-njlg5yoo | round1: spec_create(B)；round2: 无破坏动作 | 存在=true, status=draft | 0 | ✅ PASS | 0.3694 |
| **小计** | | | | **5/5 PASS** | | **1.9071** |

> 补跑说明：首轮 1/5 后按 $4 闸截断；复审后追加批（≤$2）完成剩余 4 reps（总成本 $1.5124）。**5/5 PASS：round1 均成功建 B，round2 均无归档/删除/回滚动作（sidecar 判定，B 保持存在、status=draft）——E-06b sha-c "自动归档刚建 B" 未出现于任何样本（对照 sha-a 3/5、sha-b 3/5 的 2 个 FAIL 均为 round2 归档 B）**。
...
| E-06b | 3/5 | 3/5 | **5/5** | ✅ 优于 sha-a/b（同场景 0 个归档 B FAIL；n=5 全样本） |
...
**E-06a/b "自动归档刚建 B" 是否仍现**：E-06a 4/5 仍现（与 sha-a/b 同源，B3 修复未消解——预期内，G5 修复在 sha-c 冻结之后不在快照）；E-06b 0/5 未现（sha-c 5/5 全 PASS，对照 sha-a/sha-b 各 2/5 FAIL 均为该模式）。
...
- validator strict（15 个 evidence .json，逐批）: **0 ERROR / exit 0**（warnings 为既有类 user_decision_override 恒 true 提示）
- git_sha=918581e（40hex）15/15；sha_label=sha-c 15/15；client 2.1.228 / sonnet-5 15/15；session JSONL 非空；E-06a/E-06b 均含 round1+round2 独立 jsonl + 合并 session（两轮完整入录）
- 成本（有效 sessions）：E-02 $1.5877 + E-06a $1.7347 + E-06b $1.9071 = **$5.2295**
  - E-06b 补跑 4 reps 成本 $1.5124（追加批 ≤$2 闸内）
  - 另有探针会话 1 个（harness git_sha 补丁前产出，证据已删）约 $0.29 及并行 sibling（opencode）会话成本不计入本批
- 纪律：证据未 commit（留待 DeepSeek 复审）；证据文件未手改；sha-c worktree 未改动（仍 918581e）；隔离断言由 harness LRNEV_WORKSPACE=fixture 临时工作区保证（session_clean=true 15/15）；429 未触发

## 4. 证据文件路径清单（50 文件，untracked，registry `.evidences`）
...
- E-06b×5（每 run：`-session.jsonl`/`-round1.jsonl`/`-round2.jsonl`/`-e06b.jsonl` + `.json`）：`e-06b-1788511948486-3clcp016`、`e-06b-1788512172014-lez3l2wc`、`e-06b-1788512220269-8l2ftmcy`、`e-06b-1788512298639-iaraomst`、`e-06b-1788512366324-njlg5yoo`
- 同目录另有 sibling opencode sha-c 会话（client 1.18.18，如 `e-02-…-du7xi7g2/ir2tl2dw/vnd5tjyd/nkseomer/51f4j4ho/bpr4z7ef`）——非本批归属，未纳入任何统计
