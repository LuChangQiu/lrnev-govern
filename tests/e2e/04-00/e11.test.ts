/**
 * 04-00 E-11: other 协议 - 测试
 *
 * 验证：other direction 不触发自动方向比较，不被服务端升级为约束。
 */

import { describe, test, expect } from 'vitest';
import { E11_OtherProtocol } from '../../fixtures/04-00/e11-other-protocol';

describe('E-11: other 协议', () => {
  test('fixture 定义完整性', () => {
    const fixture = E11_OtherProtocol;

    // 验证基本字段
    expect(fixture.id).toBe('E-11');
    expect(fixture.title).toBe('other 协议');
    ...
    expect(fixture.expectedDecisionContext!.direction).toBe('other');
    ...
    // 验证预期动作
    expect(fixture.expectedAction).toBeNull();
    ...
    // 验证证据字段
    expect(fixture.evidenceFields.action_taken).toBeNull();
    expect(fixture.evidenceFields.tool_sequence).toEqual([]);
  });
  ...
  test('B0 冒烟：证据采集器可填充字段', async () => {
    ...
    const evidence = await runFixture(E11_OtherProtocol);
    ...
    // 验证动作记录
    // E-11 的 fixture.expectedAction 为 null（本就不期望任何动作），无调用即为正确行为
    expect(evidence.fixture_hash).toBeDefined();
    expect(evidence.decision_context?.strength).toBe('explicit');
    expect(evidence.decision_context?.direction).toBe('other');
