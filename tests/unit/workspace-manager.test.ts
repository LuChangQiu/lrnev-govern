 ...
 * steering 写入与 PROJECT/ARCHITECTURE 的静态 FILL 骨架引导。
 */
...
describe('WorkspaceManager', () => {
  ...
  it('应在空目录中初始化完整 .lrnev 工作区', async () => {
    ...
    expect(fs.exists('.lrnev/steering/CONTEXT_DOCS_TRIGGERS.md')).toBe(true);
    ...
    expect(await fs.read('.lrnev/ARCHITECTURE.md')).toContain('<!-- FILL: 技术栈；请读项目的构建/清单文件与核心源码核实 -->');
    expect(await fs.read('.lrnev/ARCHITECTURE.md')).toContain('<!-- FILL: 主要模块/源码目录；请读源码核实 -->');
  ...
  it('F-09: 已有代码时 ARCHITECTURE 仍写静态 FILL 哨兵并引导 AI 读构建文件自行判断', async () => {
    ...
    // 无自动探测：不写任何"疑似候选"，统一留静态 FILL 由 AI 核实补全
    expect(architecture).toContain('<!-- FILL: 技术栈；请读项目的构建/清单文件与核心源码核实 -->');
    expect(architecture).toContain('<!-- FILL: 主要模块/源码目录；请读源码核实 -->');
    expect(architecture).not.toContain('自动探测疑似候选');
    expect(architecture).not.toContain('### 技术栈\n\n- TODO');
    // 新建骨架时 ai_followup 引导 AI 读构建文件自行判断技术栈与架构
    expect(followup).toContain('请读项目的构建/清单文件');
    expect(followup).toContain('自行判断技术栈与架构');
    expect(followup).toContain('PROJECT.md');
    expect(followup).not.toContain('codebase');
  });

  it('F-10-01: Java/Maven 项目(pom.xml + backend/frontend，无 src/lib/app)init 仍生成 FILL 骨架并引导 AI 识别', async () => {
    ...
    // 生态无关：lrnev 不识别具体技术栈，统一留静态 FILL，由 AI 读构建文件自行判断
    expect(res.ok).toBe(true);
    expect(architecture).toContain('<!-- FILL: 技术栈；请读项目的构建/清单文件与核心源码核实 -->');
    expect(followup).toContain('请读项目的构建/清单文件');
    expect(followup).toContain('自行判断技术栈与架构');
